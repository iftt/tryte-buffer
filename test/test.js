...
test('test encoding tryte limit', function (t) {
  ...
  const tryteEncoding = tryteBuffer.encode(testInput)
  const decodedTrytes = tryteBuffer.decode(tryteEncoding)
  ...
  t.equal(JSON.stringify(testInput), JSON.stringify(decodedTrytes), 'the decoded object is the same as the original object even though it is exceeding the limit')
})
